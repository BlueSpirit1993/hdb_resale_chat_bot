...
month_transformer = FunctionTransformer(
    encode_month_sin_cos, feature_names_out=lambda x: ['month_sin', 'month_cos']
)
...
preprocessor = ColumnTransformer(
    transformers=[
        ...
        ('floor_area_passthrough', 'passthrough', ['floor_area_sqm']),  # Pass through without scaling
...
def preprocess_data(X):
    ...
    # Construct feature names list in correct order to match transformed data
    feature_names = (
        ['flat_type_ord', 'floor_category_ord'] +
        list(ohe_region_features) +
        list(ohe_town_features) +
        ['floor_area_sqm'] +  # Original column name for passthrough
        ['month_sin', 'month_cos'] +
        ['salesYear_fr_1990']
    )
